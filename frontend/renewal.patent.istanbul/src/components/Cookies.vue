<template>
  <div class="cookie fixed-bottom" id="cookie">
    <div class="alert text-center cookiealert show q-pa-sm" role="alert">
      <span v-html="$t('cookie.message')"></span>
      <q-toggle
        false-value="Opted out"
        :label="`Cookies : ${toggle}`"
        true-value="Allowed"
        color="green"
        v-model="toggle"
      />
      <q-btn flat round color="white" @click="cookieClose()" icon="close" />
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    toggle() {
      if (this.toggle === "Allowed") {
        document.cookie = "cookie=1";
      } else if (
        this.toggle === "Opted out" &&
        document.cookie === "cookie=1"
      ) {
  ...
  mounted() {
    this.showCookie = this.isShow;
    if (document.cookie === "cookie=1") this.toggle = "Allowed";
  },
};
</script>
